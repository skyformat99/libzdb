...
#define SCHEMA_CUBRID     "CREATE TABLE zild_t(id INTEGER NOT NULL AUTO_INCREMENT, name VARCHAR(255), percent DOUBLE, image BLOB);"
...
static void testPool(const char *testURL) {
        ...
        } else if (Str_startsWith(testURL, "cubrid")) {
                schema = SCHEMA_CUBRID;
        } else {
        ...
        printf("=> Test8: Exceptions handling\n");
        {
                ...
                END_TRY;
#ifndef __APPLE__
                /* why con value is changed? mac os x bug? */
                TRY
                {
                        assert((con = ConnectionPool_getConnection(pool)));
...
int main(void) {
        ...
        char *help = "Please enter a valid database connection URL and press ENTER\n"
                    ...
                    "E.g. cubrid://localhost:33000/demodb?user=dba&password=\n"
